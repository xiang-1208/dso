...
PixelSelector::PixelSelector(int w, int h)
{
	...
	// 2021.11.30
	/*
	ths = new float[cam_num*((w/32)*(h/32)+100)];
	thsSmoothed = new float[cam_num*((w/32)*(h/32)+100)];*/
	// 2021.11.30
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{
			...
			for(int j=0;j<32;j++) for(int i=0;i<32;i++)
			{
				...
				// 2021.10.31
				// 设置阈值时去掉mask之外的点
				if(!maskG[fh->cam_idx][0][it+jt*w])	continue;
				// 2021.10.31
				// 2022.1.6 XTL:若同一个点处取到了5次，则在选择点计算阈值时不考虑该点
				// if((Var_mask[fh->cam_idx][it+jt*w]-(int)Var_mask[fh->cam_idx][it+jt*w])>=0.01)	continue;//
		...
		for(int y3=0;y3<my3;y3+=(2*pot)) for(int x3=0;x3<mx3;x3+=(2*pot))
		{
			...
			for(int y2=0;y2<my2;y2+=pot) for(int x2=0;x2<mx2;x2+=pot)
			{
				...
				for(int y1=0;y1<my1;y1+=1) for(int x1=0;x1<mx1;x1+=1)
				{
					...
					// 若在mask外，实际上梯度会很小，不需要考虑的
					// 2021.12.13
					if(!(maskG[fh->cam_idx][0][xf + yf*w])) continue;
					// 2021.12.13

					// 2022.1.6 XTL:若同一个点处取到了5次，则在选择点计算阈值时不考虑该点
					// if((Var_mask[fh->cam_idx][xf + yf*w]-(int)Var_mask[fh->cam_idx][xf + yf*w])>=0.01)	continue;//
