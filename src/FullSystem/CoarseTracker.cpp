...
CoarseTracker::CoarseTracker(int ww, int hh) : lastRef_aff_g2l(0,0)
{
	...
	// 2021.12.20
	// warped buffers
	buf_warped_X = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete); 
	buf_warped_Y = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete); 
	buf_warped_Z = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete); 
	//
	// 2021.11.24
    buf_warped_idepth = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
    buf_warped_u = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
    buf_warped_v = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
    buf_warped_dx = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
    buf_warped_dy = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
    buf_warped_residual = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
    buf_warped_weight = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
    buf_warped_refColor = allocAligned<4,float>(cam_num*ww*hh, ptrToDelete);
	// 2021.11.24
...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians)
{
	...
	for(int idx=0;idx<cam_num;idx++)
	{
		...
		{	
			// printf("frame[%d]'s ph size is:%d\n", idx, fh->frame[idx]->pointHessians.size());
			for(PointHessian* ph : fh->frame[idx]->pointHessians)
			{
				if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN)
				{
					...
					assert(r->efResidual->isActive() && r->target == lastRef->frame[idx]);
		...
		// dilate idepth[idx] by 1 (2 on lower levels).
...
void CoarseTracker::calcGSSSE(int lvl, Mat88 &H_out, Vec8 &b_out, const SE3 &refToNew, AffLight aff_g2l)
{
	...
	for(int idx=0;idx<cam_num;idx++)
	{
		...
		int n = buf_warped[idx];
		Mat33f Rx0 = T_c0_c[idx].rotationMatrix().transpose().cast<float>();
		__m128 R[3*3];
		for(int i=0;i<9;i++)
		...
		for(int i=lastn;i<n;i+=4)
		{
			...
			__m128 r1,r2,r3,rx,ry;
			for(int i=0;i<3;i++){
				// id*(R00-u*R20) id*(R01-u*R21) id*(R02-u*R22)
				rx = _mm_mul_ps(id,_mm_sub_ps(R[i],_mm_mul_ps(u,R[6+i])));
				// id*(R10-v*R20) id*(R11-v*R21) id*(R12-v*R22)
				ry = _mm_mul_ps(id,_mm_sub_ps(R[3+i],_mm_mul_ps(v,R[6+i])));
				J[i] = _mm_add_ps(_mm_mul_ps(rx,dx),_mm_mul_ps(ry,dy));
				// Y*R00-X*R10 Y*R01-X*R11 Y*R02-X*R12
				r1 = _mm_sub_ps(_mm_mul_ps(Y,R[i]),_mm_mul_ps(X,R[3+i]));
				// Z*R10-Y*R20 Z*R11-Y*R21 Z*R12-Y*R22
				r2 = _mm_sub_ps(_mm_mul_ps(Z,R[3+i]),_mm_mul_ps(Y,R[6+i]));
				// (-Z*R00+X*R20) (-Z*R01+X*R21) (-Z*R02+X*R22)
				r3 = _mm_sub_ps(_mm_mul_ps(X,R[6+i]),_mm_mul_ps(Z,R[i]));
				// id*(r4-u*r1) id*(r4-u*r2) id*(r4-u*r3)
				rx = _mm_sub_ps(_mm_mul_ps(id,r2),_mm_mul_ps(_mm_mul_ps(id,u),r1));
				// id*(r5-v*r1) id*(r5-v*r2) id*(r5-v*r3)
				ry = _mm_sub_ps(_mm_mul_ps(id,r3),_mm_mul_ps(_mm_mul_ps(id,v),r1));
				J[i+3] = _mm_add_ps(_mm_mul_ps(rx,dx),_mm_mul_ps(ry,dy));
			}
			J[6] = _mm_mul_ps(a,_mm_sub_ps(b0, _mm_load_ps(buf_warped_refColor+i)));
			J[7] = minusOne;
			J[8] = _mm_load_ps(buf_warped_residual+i);
			J[9] = _mm_load_ps(buf_warped_weight+i);
			acc.updateSSE_eighted(J
					/*_mm_mul_ps(id,dx),
					_mm_mul_ps(id,dy),
...
Vec6 CoarseTracker::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH)
{
	...
	for(int idx=0;idx<cam_num;idx++){
		...
		SE3 refToNew_tmp = T_c0_c[idx].inverse() * refToNew * T_c0_c[idx];
		...
		for(int i=0;i<nl;i++)
		{
			...
			// 2021.11.25
			if(!maskG[idx][lvl][(int)Ku+(int)Kv*wl]) continue;
			// 2021.11.25
			...
			else
			{
				...
				// 2021.12.20
				SE3 tmp = T_c0_c[idx];
				Mat33f R = tmp.rotationMatrix().cast<float>();
				Vec3f t = tmp.translation().cast<float>();
...
void CoarseTracker::debugPlotIDepthMap(float* minID_pt, float* maxID_pt, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
...

	{
		...
		{
			if(idepth[0][lvl][i] > 0)
				allID.push_back(idepth[0][lvl][i]);
		}
		...
		// 2021.11.5
		std::vector<MinimalImageB3*> _mf(CAM_USE);
		for(int i=0;i<cam_num;i++){
		...
		if(debugSaveImages)
		{
			...
			for(int i=0;i<cam_num;i++)
			{
				snprintf(buf, 1000, "images_out/predicted_%05d_%d_%05d.png", lastRef->shell->id, i, refFrameID);
		...
		FrameHessian* frame, int cam_idx)
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		...
		// 2022.1.9
		SE3 fhToNew = frame->frame[cam_idx]->PRE_worldToCam * fh->frame[cam_idx]->PRE_camToWorld;
		//
		...
		// 2022.1.6
		// std::cout<<"In makeDistanceMap, frame["<<cam_idx<<"]'s points num is:"<<fh->frame[cam_idx]->pointHessians.size()<<std::endl;
		for(PointHessian* ph : fh->frame[cam_idx]->pointHessians)
		{
