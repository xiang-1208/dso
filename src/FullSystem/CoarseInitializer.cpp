...
CoarseInitializer::CoarseInitializer(int ww, int hh) : thisToNext_aff(0,0), thisToNext(SE3())
{
	...
	// 2022.1.5
	JbBuffer = new Vec10f[cam_num*ww*hh];
	JbBuffer_new = new Vec10f[cam_num*ww*hh];
	// 2022.1.5
...
CoarseInitializer::~CoarseInitializer()
{
	for(int idx = 0;idx <cam_num;idx++)
		for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
		{
			if(points[idx][lvl] != 0) delete[] points[idx][lvl];
		}
		...
		while(true)
		{
			...
			if(accept)
			{
				...
				for(int i=0;i<cam_num;i++)
					sum += numPoints[i][lvl];
				if(resNew[1] == alphaK*sum)
					snapped = true;
				/*
				else{
					std::cout<<"resNew[1] is "<<resNew[1]<<std::endl;
					std::cout<<"alphaK*sum is "<<alphaK*sum<<std::endl;
				}*/
				// 2021.11.2
				H = H_new;
				b = b_new;
...
void CoarseInitializer::debugPlot(int lvl, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
	...
	// 2022.1.2
	std::vector<MinimalImageB3*> _iRImg(CAM_USE);
	for(int idx=0;idx<cam_num;idx++)
	{
		...
		bool plot)
{
	...
	{
		Eigen::Vector3f* colorRef = firstFrame->frame[_idx]->dIp[lvl];
		Eigen::Vector3f* colorNew = newFrame->frame[_idx]->dIp[lvl];

		SE3 refToNew_tmp = T_c0_c[_idx].inverse() * refToNew * T_c0_c[_idx];

		// 2022.1.5
		SE3 ref2New_C0 = refToNew * T_c0_c[_idx];
		Mat33f RKi_C0 = (ref2New_C0.rotationMatrix() * Ki[_idx][lvl]).cast<float>();
		Vec3f t_C0 = ref2New_C0.translation().cast<float>();
		Mat33f Rn0 = T_c0_c[_idx].rotationMatrix().transpose().cast<float>();
		//

		Mat33f RKi = (refToNew_tmp.rotationMatrix() * Ki[_idx][lvl]).cast<float>();
		...
		int npts = numPoints[_idx][lvl];
		Pnt* ptsl = points[_idx][lvl];
		
		for(int i=last_n;i<last_n+npts;i++)// 2022.1.5
		{
			Pnt* point = ptsl+(i-last_n);
			...
			for(int idx=0;idx<patternNum;idx++)
			{
				...
				if(!(Ku > 1 && Kv > 1 && Ku < wl-2 && Kv < hl-2 && new_idepth > 0 && maskG[_idx][lvl][(int)Ku+(int)Kv*wl]))
				{
					isGood = false;
					break;
				}
				...
				// 2022.1.5
				Vec3f p0 = RKi_C0 * Vec3f(point->u+dx, point->v+dy, 1)/point->idepth_new + t_C0;
				//
				...
				// 2022.1.2
				dp0[idx] = new_idepth*dxInterp*(Rn0(0,0)-Rn0(2,0)*u)+new_idepth*dyInterp*(Rn0(1,0)-Rn0(2,0)*v);
				dp1[idx] = new_idepth*dxInterp*(Rn0(0,1)-Rn0(2,1)*u)+new_idepth*dyInterp*(Rn0(1,1)-Rn0(2,1)*v);
				dp2[idx] = new_idepth*dxInterp*(Rn0(0,2)-Rn0(2,2)*u)+new_idepth*dyInterp*(Rn0(1,2)-Rn0(2,2)*v);
				dp3[idx] = new_idepth*dxInterp*(p0(2,0)*Rn0(1,0)-p0(1,0)*Rn0(2,0)-u*(p0(1,0)*Rn0(0,0)-p0(0,0)*Rn0(1,0)))+
						   new_idepth*dyInterp*(-p0(2,0)*Rn0(0,0)+p0(0,0)*Rn0(2,0)-v*(p0(1,0)*Rn0(0,0)-p0(0,0)*Rn0(1,0)));
				dp4[idx] = new_idepth*dxInterp*(p0(2,0)*Rn0(1,1)-p0(1,0)*Rn0(2,1)-u*(p0(1,0)*Rn0(0,1)-p0(0,0)*Rn0(1,1)))+
						   new_idepth*dyInterp*(-p0(2,0)*Rn0(0,1)+p0(0,0)*Rn0(2,1)-v*(p0(1,0)*Rn0(0,1)-p0(0,0)*Rn0(1,1)));
				dp5[idx] = new_idepth*dxInterp*(p0(2,0)*Rn0(1,2)-p0(1,0)*Rn0(2,2)-u*(p0(1,0)*Rn0(0,2)-p0(0,0)*Rn0(1,2)))+
						   new_idepth*dyInterp*(-p0(2,0)*Rn0(0,2)+p0(0,0)*Rn0(2,2)-v*(p0(1,0)*Rn0(0,2)-p0(0,0)*Rn0(1,2)));
	...
	float alphaEnergy = alphaW*(EAlpha.A + refToNew.translation().squaredNorm() * sum);
	//

	// printf("AE = %f * %f + %f\n", alphaW, EAlpha.A, refToNew.translation().squaredNorm() * sum);

	// compute alpha opt.
	float alphaOpt;
	if(alphaEnergy > alphaK*sum)
	{
		alphaOpt = 0;
		alphaEnergy = alphaK*sum;
	}
	else
	{
		alphaOpt = alphaW;
	...
	for(int idx=0;idx<cam_num;idx++)
	{
		int npts = numPoints[idx][lvl];
		Pnt* ptsl = points[idx][lvl];
		for(int i=last_n;i<last_n+npts;i++)
		{
			Pnt* point = ptsl+(i-last_n);
			if(!point->isGood_new)
				continue;
	...
	H_out(0,0) += alphaOpt*sum;
	H_out(1,1) += alphaOpt*sum;
	H_out(2,2) += alphaOpt*sum;

	Vec3f tlog = refToNew.log().head<3>().cast<float>();
	b_out[0] += tlog[0]*alphaOpt*sum;
	b_out[1] += tlog[1]*alphaOpt*sum;
	b_out[2] += tlog[2]*alphaOpt*sum;
...
Vec3f CoarseInitializer::calcEC(int lvl)
{
	...
	for(int i=0;i<cam_num;i++)
		sum += numPoints[i][lvl];
	if(!snapped) return Vec3f(0,0,sum);
	//
	AccumulatorX<2> E;
	E.initialize();
	for(int idx=0;idx<cam_num;idx++)
...
void CoarseInitializer::propagateUp(int srcLvl)
{
	...
	for(int idx=0;idx<cam_num;idx++)
	{
		int nptss= numPoints[idx][srcLvl];
		int nptst= numPoints[idx][srcLvl+1];
		Pnt* ptss = points[idx][srcLvl];
		Pnt* ptst = points[idx][srcLvl+1];
...
void CoarseInitializer::setFirst(CalibHessian* HCalib, FrameHessian* newFrameHessian)
{
	...
	for(int idx=0;idx<cam_num;idx++)
	{
		...
		for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
		{
			...
			int npts;
			if(lvl == 0){
				npts = sel.makeMaps(firstFrame->frame[idx], statusMap,densities[lvl]*w[0]*h[0],1,false,2);
			}
			else{
				npts = makePixelStatus(firstFrame->frame[idx]->dIp[lvl], statusMapB, w[lvl], h[lvl], densities[lvl]*w[0]*h[0]);
			}

			if(points[idx][lvl] != 0) delete[] points[idx][lvl];
			points[idx][lvl] = new Pnt[npts];

			// set idepth map to initially 1 everywhere.
			int wl = w[lvl], hl = h[lvl];
			Pnt* pl = points[idx][lvl];
			int nl = 0;
...
void CoarseInitializer::makeNN(int cam_idx)
{
	...
	for(int i=0;i<pyrLevelsUsed;i++)
	{
		pcs[i] = FLANNPointcloud(numPoints[cam_idx][i], points[cam_idx][i]);
