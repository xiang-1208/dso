...
{
	int nres = 0;
	// XTL：遍历所有关键帧，若该帧不是该点的主帧，则对residuals进行初始化，state_NewEnergy=state_energy=0,state_NewState为外点，state_state为IN，目标帧为该帧
	// XTL：简而言之，就是初始化该点和其他帧的残差项
	for(FrameHessian* fh : frameHessians)
	{
		if(fh->frame[point->host->cam_idx] != point->host)  // 2022.1.2 不创建和自己的
		{
			...
			residuals[nres].target = fh->frame[point->host->cam_idx];
			nres++;
		}
	}
	...
	// XTL：若残差的目标帧为关键帧的最后一帧/倒数第二帧（关键帧数大于1）/0（只有1个关键帧），则设置该点的lastResiduals[0/1].first=r，second为IN
	for(int i=0;i<nres;i++)
		if(residuals[i].state_state == ResState::IN)
		{
			...
			if(r->target == frameHessians.back()->frame[r->target->cam_idx]) // 2022.1.2
			{
				p->lastResiduals[0].first = r;
				p->lastResiduals[0].second = ResState::IN;
			}
			else if(r->target == (frameHessians.size()<2 ? 0 : frameHessians[frameHessians.size()-2]->frame[r->target->cam_idx]))// 2022.1.2
			{
