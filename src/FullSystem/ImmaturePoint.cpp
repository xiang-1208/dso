...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
	...
	if(!(uMin > 4 && vMin > 4 && uMin < wG[0]-5 && vMin < hG[0]-5 && maskG[frame->cam_idx][0][(int)uMin+(int)vMin*wG[0]])) // 2022.1.5
	{
	...
	if(std::isfinite(idepth_max))
	{
		...
		if(!(uMax > 4 && vMax > 4 && uMax < wG[0]-5 && vMax < hG[0]-5 && maskG[frame->cam_idx][0][(int)uMax+(int)vMax*wG[0]]))// 2022.1.5
		{
		...
		float idepth)
{
	...
	projectPoint(this->u,this->v, idepth, 0, 0,HCalib,
			precalc->PRE_RTll[0],PRE_tTll, drescale, u, v, Ku, Kv, KliP, new_idepth, 0);
		...
		float idepth)
{
	...
	{
		float Ku, Kv;
		if(!projectPoint(this->u+patternP[idx][0], this->v+patternP[idx][1], idepth, PRE_KRKiTll, PRE_KtTll, Ku, Kv, 0))
			{return 1e10;}
		...
		float idepth)
{
	...
	FrameFramePrecalc* precalc = &(host->frame[0]->targetPrecalc[tmpRes->target->frame[0]->idx]);
	
	// check OOB due to scale angle change.
	float energyLeft=0;
	const Eigen::Vector3f* dIl = tmpRes->target->dI;
	// 2021.11.9
	const Mat33f &PRE_RTll = precalc->PRE_RTll[_idx];
	const Vec3f &PRE_tTll = precalc->PRE_tTll[_idx];
	// 2021.11.9
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		if(!projectPoint(this->u,this->v, idepth, dx, dy,HCalib,
				PRE_RTll,PRE_tTll, drescale, u, v, Ku, Kv, KliP, new_idepth, _idx))
			{tmpRes->state_NewState = ResState::OOB; return tmpRes->state_energy;}
		...
		// depth derivatives.
		// 2021.11.9
		float dxInterp = hitColor[1]*HCalib->fxl(_idx);
		float dyInterp = hitColor[2]*HCalib->fyl(_idx);
		// 2021.11.9
