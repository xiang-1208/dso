		...
		float &Ku, float &Kv, int cam_idx)
{
	...
	Ku = ptp[0] / ptp[2];
	Kv = ptp[1] / ptp[2];
	return Ku>1.1f && Kv>1.1f && Ku<wM3G && Kv<hM3G && maskG[cam_idx][0][(int)Ku+(int)Kv*wG[0]];
}
...
{
	KliP = Vec3f(
			(u_pt+dx-HCalib->cxl(cam_idx))*HCalib->fxli(cam_idx),
			(v_pt+dy-HCalib->cyl(cam_idx))*HCalib->fyli(cam_idx),
			1);
	...
	u = ptp[0] * drescale;
	v = ptp[1] * drescale;
	Ku = u*HCalib->fxl(cam_idx) + HCalib->cxl(cam_idx);
	Kv = v*HCalib->fyl(cam_idx) + HCalib->cyl(cam_idx);
