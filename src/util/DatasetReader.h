...
{
public:
	ImageFolderReader(std::string path, std::string calibFile, std::string gammaFile, std::string vignetteFile, std::string undistortFile, std::string extrinsicfile, int cam_idx)
	{
		this->path = path;
		this->calibfile = calibFile;
		
		this->extrinsicfile = extrinsicfile;
	...
	// 2021.11.8
	void setExtrinsic(int cam_idx)
	{
		printf("Reading Extrinsic from file %s",extrinsicfile.c_str());

		std::ifstream f(extrinsicfile.c_str());
		if (!f.good())
		{
			f.close();
